<?php if(!empty($data_comment)){ foreach($data_comment as $k=>$v){ ?>
    <div class="media media-shadow">
        <div class="media-left">
            <a href="#">
                <img class="media-object" src="<?php echo '/uploads/headpic/'.(empty($v['member_headpic'])?'default.png':$v['member_headpic']);?>" data-holder-rendered="true">
            </a>
        </div>
        <div class="media-body">
            <?php if(!empty($v['replay_name'])){ ?>
            <small><a href="">@ <?php echo $v['replay_name'];?></a></small>
            <?php } ?>
            <?php echo empty($v['content'])?'':$v['content'];?>
            <div class="sm">
                <small>
                    <a href="javascript:void(0);"><?php echo empty($v['member_name'])?'':$v['member_name'];?></a>
                    <span class="ml10"><?php echo $v['date_add'];?></span>
                    <a class="sm ml10" href="javascript:comment_up(<?php echo $v['id'];?>)" title="赞"><li class="fa fa-thumbs-up <?php if(!empty($data_comment_up)){ foreach($data_comment_up as $u){ if($u['comment_id'] == $v['id']){ ?>primary-block<?php break; }}} ?>" id="comment_up_fa_<?php echo $v['id'];?>"></li></a>
                    <span id="comment_up_num_<?php echo $v['id'];?>"><?php echo empty($v['up'])?'':$v['up'];?></span>
                    <a class="sm fr" href="javascript:comment_replay(<?php echo $v['id'];?>)">回复</a>
                </small>
            </div>
